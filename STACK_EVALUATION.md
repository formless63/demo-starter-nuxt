...
- **Nitro/Docker:** Multi-stage output is small and portable. Compose now proves the complete provider-neutral release path with PostgreSQL, a one-shot migration job, and a health-checked application. The build bundles a minimal Node-targeted Drizzle migrator beside Nitro, allowing migration and runtime to use the same non-root image without shipping the development dependency tree. The container build disables Nuxt's redundant in-build typechecker because Vue module resolution fails inside the Bun image; the separate strict CI typecheck remains mandatory before the image build. Migrations remain an explicit release gate and never run from normal application startup.
